...
function deviceToEndpoints(device) {
  ...
  if( device.mappings.ModeController ) {
    if( d.displayCategories.indexOf('TV') === -1 )
      d.displayCategories.push ( 'TV' );
    log.debug( device.mappings.ModeController );
    let operations = [device.mappings.ModeController];
    if( typeof device.mappings.ModeController.value2homekit === 'object' )
      for( var operation in device.mappings.ModeController.value2homekit )

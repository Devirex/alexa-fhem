...
function deviceToEndpoints(device) {
  ...
  if( device.isOfType('blind') && device.mappings.TargetPosition ) {
    ...
    log.error('Mapping Invert:' + mapping.invert?'true':'false');
  ...
  if( device.mappings.ModeController ) {
    if( d.displayCategories.indexOf('TV') === -1 )
      d.displayCategories.push ( 'TV' );
    log.debug("Mode:" + device.mappings.ModeController[0]);
  
    device.mappings.ModeController.forEach(function(ModeController){
      var assets = []; 
      var asset = ModeController.asset.split(';')
      var a_name = asset[0];
      assets.push({
        "@type": "asset",
        "value": {
      ...
      asset.forEach(function(asset,i){
        if(i > 0){
          log.debug("Index:" + i);
          var a_utterance = asset.split(':')[1];
          var a_locale = asset.split(':')[0];
          assets.push({
                        "@type": "text",
                        "value": {
                          "text": a_utterance,
      ...
      var modes = [];
      ModeController.modes.forEach(function(mode){
        var mode = mode.split(';');
        var m_name = mode[0];
        var fnames = [];
        mode.forEach(function(fname, i){
          if(i > 0){      
            var m_utterance = fname.split(':')[1];
            var m_locale = fname.split(':')[0];
            fnames.push({
                        "@type": "text",
                        "value": {
                            "text": m_utterance,
...
var handleMode = function(event) {
  ...
  var mapping = device.mappings.ModeController;

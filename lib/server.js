...
const NAMESPACE_KeypadController = "Alexa.KeypadController";
...
function deviceToEndpoints(device) {
  ...
  if( device.mappings.KeypadController ) {
    if( d.displayCategories.indexOf('TV') === -1 )
      d.displayCategories.push ( 'TV' );
    let operations = [];
    if( typeof device.mappings.KeypadController.value2homekit === 'object' )
      for( var operation in device.mappings.KeypadController.value2homekit )
        operations.push( operation );
    d.capabilities.push( {
                           "type": "AlexaInterface",
                           "interface": NAMESPACE_KeypadController,
                           "version": "3",
                           "keys": operations
                         }
                       );

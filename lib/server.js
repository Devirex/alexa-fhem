...
function deviceToEndpoints(device) {
  ...
  if( device.mappings.ModeController ) {
    if( d.displayCategories.indexOf('TV') === -1 )
      d.displayCategories.push ( 'TV' );
    log.debug( device.mappings.ModeController.modes );
    log.debug( typeof device.mappings.ModeController.modes );
    let operations = [];
    if( typeof device.mappings.ModeController.modes === 'object' )
      for( var operation in device.mappings.ModeController.modes )
        operations.push( operation );
    d.capabilities.push( {
                           "type": "AlexaInterface",

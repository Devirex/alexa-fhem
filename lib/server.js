...
Server.prototype.addDevice = function(device, fhem) {
  ...
  for( let characteristic_type in device.mappings ){
    if (characteristic_type === 'ModeController'){
      var test0 = device.mappings[characteristic_type];
      var test1 = Object.values(device.mappings[characteristic_type]);
      var test3 = Object.values(device.mappings[characteristic_type].instance);
...
function deviceToEndpoints(device) {
  ...
  if( device.mappings.ModeController ) {
    for (var keys of Object.keys(device.mappings.ModeController.instance)) {
      log.debug(keys + " -> " + device.mappings.ModeController.instance[keys])
      var instance = device.mappings.ModeController.instance[keys];
      var assets = []; 
